      ...
      .then(response => {
        ...
        rows.each((i, row) => {
          ...
          try {
            ...
            const sizeMatched = $('.detDesc', row)
              .text()
              .trim()
              .match(sizeRegex);
            const seeds = Number.parseInt(
              $('[align="right"]', row)
                .eq(0)
                .text()
            );
            const leeches = Number.parseInt(
              $('[align="right"]', row)
                .eq(1)
                .text()
            );
            const category = $('.vertTh a', row)
              .eq(0)
              .text();
            const subCategory = $('.vertTh a', row)
              .eq(1)
              .text();
            ...
            let date;
            let fullDateMatched = $('.detDesc', row)
              .text()
              .trim()
              .match(fullDateRegex); // TODO: Fix whitespace
            let partialDateMatched = $('.detDesc', row)
              .text()
              .trim()
